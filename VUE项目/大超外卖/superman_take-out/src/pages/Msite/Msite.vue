    ...
    <GshopHeader :title="address.name?address.name:'定位中...'">
      ...
      <template v-slot:right>
         <span class="header_login">
          <span class="header_login_text">{{ username?username:"登录/注册"}}</span>
        </span>
      </template>
    </GshopHeader>
    <!--首页导航-->
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      username(){
          var str = this.user.name?this.user.name:this.user.phone
          if(str && str.length>5){
            // this.user.name?this.user.name.substring(0,5):this.user.phone.substring(0,5)
